...
- *Search Index Updates* - Keep Elasticsearch or other search engines synchronized with your database
...
- *Debezium Engine with Connector* - Run Debezium connectors directly inside your Quarkus application
- *No Kafka Required* - Consume change events without deploying Kafka infrastructure
- *Dev Services* - Automatically provision databases with CDC enabled during development
- *Native Support* - Full GraalVM native compilation support
...
In the same way, databases that support schema changes (DDL), Debezium needs to track the schema evolution.
...
- *key*: The record key (usually the primary key of the changed row)
- *value*: The change event envelope with before/after states
...
[WARNING]
====
By default, any uncaught exception will cause the connector to stop.
...
Quarkus provides Dev Services for Debezium, which automatically provisions a database with CDC enabled during development.
...
1. Ensure connector is running and consuming events
